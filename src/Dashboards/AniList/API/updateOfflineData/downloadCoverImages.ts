...
export default function downloadCoverImages(type: "ANIME" | "MANGA", size: "extraLarge" | "large",
  ...
  notificationsContainer.append(
    NotificationCard(`Check & Update All ${type} ${size} Cover Image`, notificationSettings)
  );
  console.log(
  ...
  const coverSize = size === "extraLarge" ? "CoverImageExtraLargeSize" : "CoverImageLargeSize"

  AniList_API(coverSize, variables)
    .then(({ data }: any) => {
      console.log(`API DATA ${type} Cover Images`, data);
      const lists = data.MediaListCollection.lists;


      const downloadedCoverImages: Array<string> = window.DATA.readDir(`anilist/media/${type.toLowerCase()}/cover-image/${size}`)
      ...
      for (const [, list] of Object.entries(lists)) {
        for (const listItme of list.entries) {
          const imgURL = size === "extraLarge" ? listItme.media.coverImage.extraLarge : listItme.media.coverImage.large


          if (imgURL) {

            const imgFileName = size === "extraLarge" ? String(imgURL.match(/(?<=large\/).*/g)) : String(imgURL.match(/(?<=medium\/).*/g))
            ...
            if (!downloadedCoverImages.includes(imgFileName)) {
              ...
              setTimeout(() => {
                ...
                notificationsContainer.append(NotificationCard(`Download Complete ${type} ${size} size Cover\n${animeOrMangaTitle}`, notificationSettings))
              }, 1000 * i)
            }
          } // END if(imgURL)
        }
