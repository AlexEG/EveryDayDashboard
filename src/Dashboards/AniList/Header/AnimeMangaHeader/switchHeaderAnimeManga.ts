...
export default function switchHeaderAnimeManga(
  type: "anime" | "manga",
  ...
  animeMangaOfflineData.then((offlineData: any) => {
    ...
    function changeBannerImgSrc() {
      ...
      if (isAnimeHeaderThere) {
        ...
        const coverImgFileNameLarge: string = headerData.coverImgFileNameLarge;
        const coverImgFileNameExtraLarge: string =
          headerData.coverImgFileNameExtraLarge;
        const popularity: number = headerData.popularity;
        const averageScore: number = headerData.averageScore;
        const meanScore: number = headerData.meanScore;
        const favourites: number = headerData.favourites;
        ...
        const seasonYear: string = headerData.seasonYear;
        ...
        if (!isFavourite && !isFavouriteSVG.classList.contains("hidden"))
          isFavouriteSVG.classList.add("hidden");
        ...
        if (type === "manga") {
          const { volumes: Vol, chapters: Ch } =
            infoBanner as mangaHeaderInfoBannerTypes;
